...
\def\degree    {碩士} % 碩士/博士
\def\studentID {105022511} % XD
\def\titleZh   {利用ATLAS探測器的MonoHbb分析之背景估計研究}   % Chinese title
\def\titleEn   {Studies of the Background Estimation in the MonoHbb Analysis with the ATLAS Detector} % English title
...
\def\profsZh   {徐百嫻} 
\def\profsEn   {Pai-Hsien Jennifer Hsu}
\def\degreedate{中~華~民~國~ \zhdigits{108} ~年~一~月} % NTHU demo use O not 零
\def\copyyear  {2012-2013}

\setboolean{printcopyright}{true} % {true}/{false} print copyright text on titlepage or cover.

\def\keywordsZh{大型強子對撞機, ATLAS探測器, 希格斯玻色子, 暗物質}
\def\keywordsEn{Large Hadron Collier, ATLAS Detector, Higgs boson, dark matter}
...
\def\bibStyle{nature} % Default ``ieee'' with BibLaTeX. If you use BibTeX change to ``ieeetr''.
...
% Linux 利用指令 fc-list :lang=en 來查詢可以用的字體名稱。
